    ...
    swipedetect(el, callback) {
      ...
      touchSurface.addEventListener('touchend', function(event) {
        ...
        if (elapsedTime <= allowedTime) { // first condition for awipe met
          if (Math.abs(distX) >= minDist && Math.abs(distY) <= maxDist) { // 2nd condition for horizontal swipe met
            swipedir = (distX < 0) ? 'left' : 'right' // if dist traveled is negative, it indicates left swipe
          } else if (Math.abs(distY) >= minDist && Math.abs(distX) <= maxDist) { // 2nd condition for vertical swipe met
